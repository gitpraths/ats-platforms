...
export default function PlacementDetail() {
  ...
  function getCheckStatus(wc: WelfareCheck) {
    ...
    return { label: "Pending", cls: "text-gray-600 bg-gray-50" };
  }

  return (
    <div className="p-6 max-w-4xl mx-auto">
      <Link to="/placements" className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 mb-6">
        ...
        <div>
          ...
          {placement.employer_name && (
            <p className="text-sm text-gray-500 mt-1">
              <Link to={`/employers/${placement.employer_id}`} className="hover:underline text-blue-600">
                {placement.employer_name}
              </Link>
            </p>
          )}
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ml-2 ${
              placement.confirmed_by_employer ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"
            }`}>
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white border rounded-xl p-4">
          <p className="text-xs font-semibold text-gray-400 uppercase mb-2">Candidate</p>
          <Link to={`/candidates/${placement.candidate_id}`}
            className="font-medium text-blue-600 hover:underline text-sm">{placement.candidate_name}</Link>
          {placement.candidate_work_status && (
            <p className="text-xs text-gray-500 mt-1">{placement.candidate_work_status.replace("_", " ")}</p>
          )}
          {placement.provider_name && (
            <p className="text-xs text-gray-500 mt-0.5">via {placement.provider_name}</p>
          )}
        </div>
        <div className="bg-white border rounded-xl p-4">
          <p className="text-xs font-semibold text-gray-400 uppercase mb-2">Job</p>
          <Link to={`/jobs/${placement.job_id}`}
            className="font-medium text-blue-600 hover:underline text-sm">{placement.job_title}</Link>
          {placement.employer_name && (
            <p className="text-xs text-gray-500 mt-1">{placement.employer_name}</p>
          )}
        </div>
        <div className="bg-white border rounded-xl p-4">
          <p className="text-xs font-semibold text-gray-400 uppercase mb-2">Placement</p>
      ...
      <div className="bg-white border rounded-xl p-6">
        <h2 className="font-semibold text-gray-900 mb-5">Welfare Check Timeline</h2>
        <div className="space-y-4">
          {wcs.map((wc) => {
            const status = getCheckStatus(wc);
            return (
              <div key={wc.id} className={`flex gap-4 p-4 rounded-xl border ${
                wc.due_date <= today && !wc.completed_at ? "border-yellow-200 bg-yellow-50/30" : "border-gray-100"
              }`}>
                <div className="flex-shrink-0 mt-0.5">{getCheckIcon(wc)}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-medium text-gray-900 text-sm">{CHECK_LABELS[wc.check_type as WelfareCheckType]}</p>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${status.cls}`}>{status.label}</span>
                  </div>
                  <p className="text-xs text-gray-500 mt-0.5">Due: {format(new Date(wc.due_date), "MMM d, yyyy")}</p>
                  {wc.completed_at && (
                  ...
                  {wc.employer_response && (
                    <p className="text-sm text-gray-700 mt-2 italic">"{wc.employer_response}"</p>
                  )}
                  {wc.email_sent_at && (
      ...
      {completeId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Mark Welfare Check Complete</h2>
            <div className="mb-4">
              <label className="block text-xs font-medium text-gray-600 mb-1">Employer Response / Notes</label>
