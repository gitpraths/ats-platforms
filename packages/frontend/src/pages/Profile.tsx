...
const ROLE_BADGE: Record<string, string> = {
  admin:           "border border-red-400 text-red-500 bg-transparent",
  recruiter_admin: "border border-orange-400 text-orange-600 bg-transparent",
  hiring_manager:  "border border-purple-400 text-purple-600 bg-transparent",
  recruiter:       "border border-blue-400 text-blue-600 bg-transparent",
  user:            "border border-slate-400 text-slate-600 bg-transparent",
};
...
export default function Profile() {
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      <h1 className="text-3xl font-semibold text-slate-900 tracking-tight mb-6">My Profile</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-4 flex items-center gap-5">
        <div className="relative">
          {avatarSrc ? (
            ...
            <div className="w-20 h-20 rounded-full border border-blue-400 text-blue-600 bg-transparent flex items-center justify-center text-2xl font-bold">
              {initials}
            </div>
          )}
          <button
            onClick={() => fileRef.current?.click()}
            className="absolute bottom-0 right-0 bg-white border rounded-full p-1.5 shadow hover:bg-slate-50"
          >
            <Camera size={14} />
          </button>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h2 className="font-semibold text-slate-900 tracking-tight mb-4">Personal Info</h2>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mt-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-semibold text-slate-900 tracking-tight">Account Info</h2>
        </div>
        <div className="space-y-3 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-slate-400 font-mono text-xs truncate max-w-[220px]">{user?.id ?? "—"}</span>
          </div>
        </div>
      </div>
    </div>
