  ...
  try {
    ...
    const { rows: apps } = await pool.query(
      `SELECT a.id, a.stage, a.source, a.score, a.applied_at,
              ...
              p.termination_reason AS placement_termination_reason
       FROM applications a
...
candidatesRouter.post("/", requireRole("admin", "recruiter_admin", "recruiter"), async (req, res, next) => {
  try {
    const {
      ...
      date_referred, benchmark_hours, availability,
    ...
    const { rows } = await pool.query(
      ...
      [
        ...
        benchmark_hours ? Number(benchmark_hours) : null, availability || null,
        industry_preference || [],
  ...
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE candidates SET
         ...
         industry_preference = COALESCE($14, industry_preference),
         car                 = $15,
         police_check        = $16,
         wwc                 = $17,
         comments            = COALESCE($18, comments),
         notes               = COALESCE($18, notes),
         interested_job      = COALESCE($19, interested_job),
         wage_subsidy        = COALESCE($20, wage_subsidy),
